---
title: Using Storybook for prototyping 
type: cookbook
order: 11
---
## Introduction
From the official GitHub repo, Storybook "is a development environment for UI components. It allows you to browse a component library, view the different states of each component, and interactively develop and test components.".

It's also very useful to develop component in isolation and rapid prototyping.
## From 0 to 100 in one command 
The latest version of the storybook cli is so amazing that with a single command we can have everything up and runnig, let's go.
  
This exercise will assume a globally instlled vue-cli v2.9 and node >= 8.9, npm commands will not be abbreviated in favour or readability.

Create a simple Vue app with vue cli:

```sh
vue init webpack-simple storybook-example
...
once all that is running you can head to the default url the storybook provides [http://localhost:6006](http://localhost:6006), and you'll see a very nice User Interface with some pre-built components ready to test

![Storybook](/images/storybook_base.png)

A lot happened there:
- The CLI figured out we have a Vue app
- Added the dependecies required by storybook
